...
const TripReports = (props) => {
  ...
  return props.user && props.authenticated ?(
    <div className='trWrapper'>
      ...
      {handleSort.map((post) => (
        <TripReportInfo key={post.id} body={post.body} title={post.title} image={post.image} BASE_URL={BASE_URL} id={post.id} userId={post.userId} user={user} auth={props.authenticated} created={post.createdAt}/>
      ))}
      </div>
    </div>
  ):(
